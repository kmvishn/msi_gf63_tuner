...
public sealed class CommonConfig
{
    ...
    /// <summary>
    /// How verbose service logs should be.
    /// </summary>
    [XmlElement]
    public LogLevel LogLevel { get; set; } = LogLevel.Debug;

    /// <summary>
    /// Should the experimental WMI2 backend be
    /// used to apply fan settings on "gen-2" ECs?
    /// </summary>
    /// <remarks>
    /// If <see langword="false"/>, the old
    /// WinRing0-based backend will always be used.
    /// </remarks>
    [XmlElement]
    public bool UseWMI2 { get; set; }

    /// <summary>
    /// <see langword="true"/> to disable Full Blast when exiting
    /// the YAMDCC config editor, otherwise <see langword="false"/>.
    /// </summary>
    [XmlElement]
    public bool DisableFBOnExit { get; set; } = true;

    /// <summary>
    /// <see langword="true"/> if we've already asked to enable
    /// auto-updating, otherwise <see langword="false"/>.
    /// </summary>
    [XmlElement]
    ...
    public static bool GetUseWMI()
    {
        return Load().UseWMI2;
    }
    public static void SetUseWMI(bool val)
    {
        CommonConfig cfg = Load();
        cfg.UseWMI2 = val;
        cfg.Save();
