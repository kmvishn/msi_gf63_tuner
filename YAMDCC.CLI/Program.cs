...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        if (ecMonitor && ConnectSvc())
        {
            ...
            Console.WriteLine("CPU fan speed:     , temp:");
            Console.WriteLine("GPU fan speed:     , temp:");
            Console.WriteLine();
            Console.WriteLine("Fan RPMs:");
            Console.WriteLine("  #0:");
            Console.WriteLine("  #1:");
            Console.WriteLine("  #2:");
            Console.WriteLine("  #3:");
            ...
            while (true)
            {
                IPCClient.PushMessage(new ServiceCommand(Command.GetTemps));
                IPCClient.PushMessage(new ServiceCommand(Command.GetFanSpeeds));
                IPCClient.PushMessage(new ServiceCommand(Command.GetFanRPMs));
                Thread.Sleep(1000);
            }
        }
        ...
        switch (e.Message.Response)
        {
            case Response.FanSpeeds:
            {
                if (args.Length == 2 && args[0] is byte cpuSpeed && args[1] is byte gpuSpeed)
                {
                    ...
                    Console.Write($" {cpuSpeed,3}%");
                    Console.SetCursorPosition(14, 3);
                    Console.Write($" {gpuSpeed,3}%");
                }
                break;
            }
            case Response.Temps:
            {
                if (args.Length == 2 && args[0] is byte cpuTemp && args[1] is byte gpuTemp)
                {
                    ...
                    Console.Write($"{cpuTemp,3} °C");
                    Console.SetCursorPosition(26, 3);
                    Console.Write($"{gpuTemp,3} °C");
                }
                break;
            }
            case Response.FanRPMs:
            {
                if (args.Length == 1 && args[0] is object[] rpmObj)
                {
                    ...
                    for (int i = 0; i < rpms.Length; i++)
                    {
                        ...
                        if (rpms[i] > -1)
                        {
                            Console.Write($"{rpms[i],4} RPM");
                        }
                        else
                        {
                            ConsoleColor color = Console.ForegroundColor;
                            Console.ForegroundColor = ConsoleColor.DarkGray;
                            Console.Write("off/not present");
